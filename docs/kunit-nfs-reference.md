...
`/mnt/nfs`. Ports that need a second mount, or a mount they can unmount
(for example 565, 604), call `xfs_scratch_mount()`: the same export again on
`/mnt/scratch` with `nosharecache`, so it gets its own superblock. The
