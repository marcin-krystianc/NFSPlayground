...
# NFS_LOOPBACK_BASE is exported with fsid=0 as the NFSv4 pseudo-root. Without
# it mountd synthesises the pseudo-root from "/", and in the container "/" is
# overlayfs, which knfsd cannot export; the client then sees ENOENT at mount
# time. Consequence: NFSv4 paths are relative to the base ("/test"), NFSv3
# paths are absolute. Same asymmetry as 00-run-xfstests-on-vm-and-docker.sh.
case "$NFS_LOOPBACK_VERS" in
    3*) NFS_EXPORT_PREFIX="$NFS_LOOPBACK_BASE" ;;
    *)  NFS_EXPORT_PREFIX="" ;;
esac
...
${NFS_LOOPBACK_BASE} ${NFS_LOOPBACK_CLIENT}(ro,sync,no_subtree_check,no_root_squash,fsid=0)
...
if ! $SUDO mount -t nfs -o "vers=${NFS_LOOPBACK_VERS}" \
        "127.0.0.1:${NFS_EXPORT_PREFIX}/test" "$probe"; then
    rmdir "$probe"
...
export FSTYP=nfs
export TEST_DEV=127.0.0.1:${NFS_EXPORT_PREFIX}/test
export TEST_DIR=${TEST_MNT}
export SCRATCH_DEV=127.0.0.1:${NFS_EXPORT_PREFIX}/scratch
export SCRATCH_MNT=${SCRATCH_MNT}
# Pin the version so xfstests mounts with the same NFS_EXPORT_PREFIX
# convention as the probe mount above (common/config: _common_mount_opts).
export NFS_MOUNT_OPTIONS="-o vers=${NFS_LOOPBACK_VERS}"
EOF
cat "$config"
