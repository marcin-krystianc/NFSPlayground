...
#
# The paths are relative to the pseudo-root (the export with fsid=0), not
# the server's real absolute path: NFSv4 has no separate mount protocol, so
# every export below fsid=0 is reached by a path relative to it (/test, not
# /export/test). NFS_MOUNT_OPTIONS defaults to vers=4.2 to match; both are
# overridable, but changing one without the other breaks the mount (v3 has
# no pseudo-root and needs the real /export/test path instead).
config="${XFSTESTS_DIR}/local.config"
log "writing $config"
cat > "$config" <<EOF
# generated by 00-run-xfstests-on-vm-and-docker.sh -- do not edit, it is overwritten
export FSTYP=nfs
export TEST_DEV=$(server_ip 1):/test
export TEST_DIR=${TEST_MNT}
export SCRATCH_DEV=$(server_ip 2):/scratch
export SCRATCH_MNT=${SCRATCH_MNT}
export NFS_MOUNT_OPTIONS="${NFS_MOUNT_OPTIONS:--o vers=4.2}"
...
status=0
set +e
# -E: plain `sudo` drops the calling environment, silently losing
# FSTYP-specific vars xfstests itself reads (NFS_MOUNT_OPTIONS for the NFS
# version/mount options, XFS_MOUNT_OPTIONS, etc. -- see common/config's
# _common_mount_opts()). Without it every NFS run mounts with whatever the
# kernel negotiates by default instead of what was actually asked for.
( cd "$XFSTESTS_DIR" && sudo -E ./check -nfs "${check_args[@]}" )
status=$?
set -e
