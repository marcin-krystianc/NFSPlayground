...
MANUAL = {
    ...
    "571": "_require_test_fcntl_setlease notruns on NFS without a delegation (common/rc: locktest -t returns EAGAIN); nfs4_add_lease() refuses a lease unless the client holds a delegation. Whether knfsd grants the read and write delegations the lease cases need was not probed",
    ...
    "452": "copies ls onto the mount and executes it, before and after a read-only remount; a KUnit case cannot exec a binary",
