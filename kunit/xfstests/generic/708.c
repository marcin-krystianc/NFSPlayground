 ...
 *
 * src/dio-buf-fault creates a file, maps it, touches only its first page,
 * and then does one O_DIRECT write of the whole mapping into a second
 * file. The write has to fault the rest of the source in as it goes, so
 * it is a direct write that completes partially and then continues --
 * the path btrfs got wrong in commit b73a6fd1b1ef ("btrfs: split partial
 * dio bios before submit").
 *
 ...
 * rest, so the port compares the whole destination file with the source
 * afterwards, on the client and on the server.
 */
...
static void a_direct_write_from_a_partly_faulted_mapping(struct kunit *test)
{
	...
	/* the source file, filled with a positional pattern */
	src = filp_open(G708_SRC, O_RDWR | O_CREAT | O_TRUNC, 0644);
	...
	addr = kunit_vm_mmap(test, src, 0, G708_SIZE, PROT_READ | PROT_WRITE,
			     MAP_SHARED, 0);
	...
	dst = filp_open(G708_DST, O_RDWR | O_CREAT | O_TRUNC | O_DIRECT, 0644);
	...
	g708_verify(test, G708_DST, buf, "client");
	g708_verify(test, G708_SERVER_DST, buf, "server");
}
