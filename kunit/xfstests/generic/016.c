...
/*
 * xfstests generic/016 over a loopback NFS mount: delayed-allocation multi collapse range.
 *
 ...
 * flags: 021 plain, 012 "-k", 022 "-d", 016 "-d -k". This is "-d -k": no fsync, and the file kept between layouts.
 *
 ...
 *
 * The "-k" half clears remove_testfile, so the scratch file survives
 * between the seventeen layouts and each is built on the previous one's
 * result. This port is cumulative to match, and asserts the carryover
 * positively (an offset a layout never wrote must still hold earlier
 * data) -- a shadow-model test would otherwise pass either way.
 *
...
#define G_ROOT		XFS_MNT "/g016"
#define G_FILE		G_ROOT "/case"
#define G_SERVER_FILE	XFS_EXPORT "/g016/case"
...
struct g_case {
	...
	/*
	 * An offset this layout does NOT write, which must therefore still
	 * hold a previous case's data -- what proves -k is in effect. -1 for
	 * layouts that define every byte themselves.
	 */
	loff_t		carry;
};
...
static const struct g_case g_cases[] = {
	...
	{ "2. into allocated space", G_20K,
	  { { 0, G_20K } }, false, { }, { G_4K, G_8K }, -1 },
	{ "4. hole -> data", G_20K,
	  { { G_8K, G_8K } }, false, { }, { G_4K, G_8K }, 0 },
	{ "6. data -> hole", G_20K,
	  { { 0, G_8K } }, false, { }, { G_4K, G_8K }, G_12K },
	{ "10. hole -> data -> hole", G_20K,
	  { { G_8K, G_4K } }, false, { }, { G_4K, G_12K }, 0 },
	{ "11. data -> hole -> data", G_20K,
	  { { 0, G_8K }, { G_12K, G_8K } }, true, { G_8K, G_4K }, { G_4K, G_12K }, -1 },
	{ "14. data -> hole @ 0", G_20K,
	  { { 0, G_20K } }, false, { }, { 0, G_8K }, -1 },
	{ "17. data -> hole in single block file", 4096,
	  { { 0, 4096 } }, false, { }, { 128, 128 }, -1 },
};

/*
 * The model: expected content and expected size. Both carry forward across cases, because -k keeps the file.
 */
static u8 *g_shadow;
...
static void delayed_cumulative_collapse_noop_and_writeback_delivers(struct kunit *test)
{
	u8 *got;
	int ci;
	...
	for (ci = 0; ci < ARRAY_SIZE(g_cases); ci++) {
		...
		/* -k in effect: a byte this layout never wrote was inherited */
		if (c->carry >= 0)
			KUNIT_ASSERT_NE_MSG(test, g_shadow[c->carry], 0,
					    "case %s: offset %lld should have been inherited from a previous layout -- is the file being recreated per case?",
					    c->name, c->carry);
...
static struct kunit_case g_cases_tab[] = {
	KUNIT_CASE(delayed_cumulative_collapse_noop_and_writeback_delivers),
	{}
};

static struct kunit_suite g_suite = {
	.name		= "xfstests/generic/016",
...
MODULE_DESCRIPTION("xfstests generic/016 over a loopback NFS mount");
MODULE_LICENSE("GPL");
